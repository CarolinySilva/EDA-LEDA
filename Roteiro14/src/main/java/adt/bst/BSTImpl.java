...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	protected int height(BSTNode<T> node) {
		...
		return Math.max(this.height((BSTNode<T>) node.getRight()),
				this.height((BSTNode<T>) node.getLeft())) + 1;
	...
	protected boolean hasOneChild(BSTNode<T> node) {
		return ((!node.getRight().isEmpty() && node.getLeft().isEmpty()) || (node
				.getRight().isEmpty() && !node.getLeft().isEmpty()));
	}
